...
function removeFirstOccurrences(str, value) {
  ...
  return str.slice(0, num).concat('', str.slice(num + value.length));
}
...
function getCardId(value) {
  ...
  const suit = ['♣', '♦', '♥', '♠'];
  let res = card.indexOf(value.slice(0, -1));
  if (value.slice(-1) !== '♣') {
    res += suit.indexOf(value.slice(-1)) * 13;
  }
  return res;
}
module.exports = {
